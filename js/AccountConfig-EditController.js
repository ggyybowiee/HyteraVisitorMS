(function() {
	app.controller("AccountConfig-EditCtrl", ["$scope", "$state", "serviceHttp", "$timeout", "serverInfo", "$stateParams", function($scope, $state, serviceHttp, $timeout, serverInfo, $stateParams) {
		console.log("AccountConfig-EditCtrl");
		
		console.log($stateParams.acEditItem);
		$scope.accountObj = {
			username: "",
			password: "",
		...
		function getBuildingList() {
			...
			serviceHttp.getBuildingList(
			    function (successData) {
		            ...
		            serviceHttp.getAccountInfo(
		                $stateParams.acEditItem.id,
		                function (successData) {
		                    $scope.accountObj.username = successData.name;
		                    $scope.accountObj.password = successData.password;
		                    for(var j = 0; j < successData.buildings.length; j++){
		                        for(var i = 0; i < $scope.buildingList.length; i++){
		                            var build = $scope.buildingList[i];
		                            if(build.buildId == successData.buildings[j].buildId){
		                                build.selected = true;
		                            }
		                        }
		                    }

		                    for(var jj = 0; jj < successData.services.length; jj++){
		                        for(var ii = 0; ii < $scope.accountObj.list.length; ii++){
		                            var obj = $scope.accountObj.list[ii];
		                            if(obj.index == successData.services[jj]){
		                                obj.selected = true;
		                            }
		                        }
		                    }
